...
//   the counterpart to crush_module_onload(): withdraw the loader, then dispose of the context
// object that loader produced. This module owns that object -- it allocated it, and the
// context loader hands the receiving loader ownership of the json document explicitly.
//
...
uint8_t crush_module_onunload(void)
{
        ...
        if(!root)
                return CODE_OK;
        struct crush_module_context *ctx = crush_module_get_context(root);
        if(!ctx)
                return CODE_OK;

        // detached first, so nothing can reach it through the context while it is coming apart
        crush_context_remove_context_object(root, CRUSH_MODULE_CONTEXT_OBJECT_NAME);
        crush_module_release_context(ctx);

        return CODE_OK;
}
//   releases everything crush_module_load_context() built, mirroring
...
void crush_module_release_context(struct crush_module_context *context)
{
        ...
        //   light_mutex_init(), not _recursive(), in crush_module_load_context() -- destroyed
        // the same way either way, but worth noting the asymmetry with its two siblings
...
void crush_module_load_context(struct crush_context *context, const uint8_t *file_path, crush_json_t *data)
{
        ...
        // kept so crush_module_onunload() can release it: this loader owns `data` from here on
        mod_ctx->data_root = data;
        ...
        mod_ctx->version = (uint16_t) version_f;
        mod_ctx->next_id = (uint32_t) next_id_f;
...
uint8_t crush_module_context_commit(struct crush_module_context *context)
{
        ...
        json_t *obj_data = json_pack(CONTEXT_OBJECT_FMT_WRITE,
                                        "version",      (double)context->version,
                                        "type",         CRUSH_MODULE_CONTEXT_OBJECT_NAME,
                                        "next_id",      (float)context->next_id,
